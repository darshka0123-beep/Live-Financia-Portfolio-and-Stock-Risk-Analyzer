...
if not df_crisis_raw.empty and len(df_crisis_raw.dropna()) > 5:
    ...
    # Calculate Peak-to_trough Loss
    port_max_loss = ((cum_crisis_port.min() - cum_crisis_port.iloc[0]) / cum_crisis_port.iloc[0]) * 100
    sp500_max_loss = ((cum_crisis_sp500.min() - cum_crisis_sp500.iloc[0]) / cum_crisis_sp500.iloc[0]) * 100

    col_c1, col_c2 = st.column(2)
    ...
    fig_crisis = px.line(
        df_crisis_cum,
        labels={"value": "Normalized Grwoth (Base 1.0)", "index": "Date"},
...
with col_f1:
    ...
    for ticker in tickers:
        try:
            ...
            fundamentals.append({"Ticker": ticker, "Name": "N/A", "Forward P/E": "N/E", "Market Cap ($B)": "N/A", "Profit Margin (%)": "N/A"})

    df_fund = pd.DataFrame(fundamentals)
    st.dataframe(df_fund, use_container_width=True)

with col_f2:
    st.markdown("Download Risk Analytics Report")

    # Export Data Summary
    report_data = pd.DataFrame({
        "Metric": ["Epxected Annual Return", "Annual Volatility", "Sharpe Ratio", "Portfolio Beta", "Max Drawdown", "Daily 95% VaR"],
        "Value": [f"{annual_return*100:.2f}%", f"{annual_volatility*100:.2f}%", f"{sharpe_ratio:.2f}", f"{portfolio_beta:.2f}", f"{max_drawdown:.2f}%",f"{var_95:.2f}%"]
    })
